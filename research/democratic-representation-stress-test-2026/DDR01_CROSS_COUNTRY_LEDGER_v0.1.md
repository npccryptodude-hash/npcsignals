# DDR-01 Cross-Country Ledger v0.1

Status: METHOD-FROZEN / EVIDENCE-OPEN  
Parent: DDR-01 // Demographic Voting Alignment  
Placement: Dimension VI — Polarization / Realignment  
Core MIS framework: UNCHANGED  
Signal status: NOT A SIGNAL

## Purpose

This ledger standardizes the DDR-01 evidence already collected across the 15-country Democratic Representation Stress Test sample.

The module measures two distinct channels:

1. DIRECT EFFECT — how immigrant-origin / minority / naturalized electorates themselves vote.
2. INDIRECT EFFECT — whether immigration or demographic exposure changes turnout or party choice in the wider electorate.

No country receives a generic "immigration effect" score.

## Frozen fields

DDR1 — Eligible-electorate share  
DDR2 — Turnout gap  
DDR3 — Party/bloc alignment  
DDR4 — Origin heterogeneity  
DDR5 — Alignment trajectory  
DDR6 — Counter-mobilization evidence  
DDR7 — Outcome relevance

Evidence tiers:
A — direct individual-level / validated / official election-study evidence  
B — strong representative subgroup survey evidence  
C — aggregate/ecological or strong indirect evidence  
D — insufficient / non-comparable for the focal measure

## Cross-country ledger

| Country | Primary population definition | Evidence tier | DDR1 electorate share | DDR2 turnout | DDR3 alignment | DDR4 heterogeneity | DDR5 trajectory | DDR6 counter-mobilization | DDR7 outcome relevance | Working pattern |
|---|---|---|---|---|---|---|---|---|---|---|
| Sweden | Foreign background / newly naturalized | A/B | Material; exact 2026 national eligible share still to lock | Lower turnout historically than Swedish-background electorate | Stronger centre-left / left-green preference than Swedish background | Present | Non-linear; narrowed by 2018, strengthened again by 2022–26 | Not yet estimated in DDR-01 | **MATERIAL, NOT CAUSALLY DECISIVE** | Alignment persistence / re-strengthening |
| Norway | Immigrants + Norwegian-born with immigrant parents | A | ~11.5% of eligible electorate in 2025 | 56% immigrants; 60% Norwegian-born with immigrant parents vs 83% reference | Aggregate left/green tilt remains | Strong by region/origin | Fragmenting; Frp gains in 2025 | Not yet causally estimated | **MATERIAL, SEAT EFFECT NOT IDENTIFIED** | Alignment fragmentation |
| Denmark | Immigrant-origin direct measure not yet locked | C for direct; strong causal indirect evidence | Open | Lower turnout documented for non-Western immigrants | Direct national estimate not yet locked | Likely, not yet standardized | Open | **SUPPORTED, CONTEXT-DEPENDENT** | No current national DDR7 | Counter-mobilization mechanism |
| Germany | Migration background / origin groups | B | ~12% benchmark of eligible electorate | Not standardized in module yet | No single national direction | **Very strong** | Changing across groups | Not yet estimated as national DDR6 | **MATERIAL ELECTORATE / NET NATIONAL DIRECTION NOT IDENTIFIED** | Origin-dependent cross-bloc heterogeneity |
| Great Britain | Ethnic-minority electorate | B | Population proxy ~16%; eligible share not yet locked | Not standardized | Strong Labour / left-liberal aggregate advantage in 2024 | **Strong**, including Muslim/Hindu/Asian divergence | Aggregate persistence + fragmentation | Not separately identified | **MATERIAL NATIONAL ELECTORATE / LOCALLY MATERIAL SUBGROUP SHIFTS / NATIONAL GOVERNMENT EFFECT NOT IDENTIFIED** | Aggregate persistence + local fragmentation |
| France | Immigrant / descendant status for turnout; private surveys needed for party choice | B/C participation; D current party-choice comparability | Strong demographic base | Usable turnout evidence | Open | Potentially measurable | Open | Open | Not assignable | Partial entry — turnout track only |
| Italy | Immigration exposure / municipal immigrant share | C direct; strong historical causal indirect evidence | Open | Open | Direct naturalized-citizen alignment not yet locked | Open | Open | **HISTORICALLY SUPPORTED / CURRENT EFFECT HETEROGENEOUS** | No current national DDR7 | Counter-mobilization with temporal saturation |
| Poland | Immigrant-origin electorate | D | Open | Open | Insufficient comparable evidence | Open | Open | Open | Not classified | Do not infer |
| United States | Naturalized citizens | A | ~10% eligible benchmark; ~9% of 2024 voters | Composition effects strongly documented | 2024 near parity: Harris 51 / Trump 47 | Present across race/ethnicity | Rapid convergence from 2020 | Not separately causal in current module | **MATERIAL REALIGNMENT / LIMITED NET NATIONAL BLOC ADVANTAGE** | Turnout-composition realignment |
| Costa Rica | Naturalized electorate | A electorate composition; D party choice | Strong administrative register data | Open | Insufficient | Origin distribution available administratively | Potentially reconstructable | Open | Not assignable | Partial entry — electorate structure only |
| Chile | Foreign-national electorate eligible under residence rules | A participation / C party-choice proxy | ~5.6% of provisional 2025 electorate (886,190 of 15,791,056) | Very strong administrative turnout evidence | Party vote unresolved; party-membership proxy only | Strong by origin and integration variables | Turnout trajectory measurable | Open | Not assignable | Strong participation case / party alignment unresolved |
| Ghana | Ethnic/regional cleavage track | Non-comparable | N/A for focal immigrant-origin measure | N/A | Not directly comparable | Strong ethnic/regional structure may exist | Separate track | Separate track | Not classified | Ethnic/regional cleavage track |
| Botswana | Ethnic/regional cleavage track | Non-comparable | N/A for focal immigrant-origin measure | N/A | Not directly comparable | Separate track | Separate track | Separate track | Not classified | Ethnic/regional cleavage track |
| Japan | Naturalized citizens; non-citizen foreign residents excluded from national vote | D direct vote-choice | Open | Insufficient | Insufficient robust national party-choice evidence | Open | Open | WATCH only for indirect salience; causal effect unidentified | Not classified | Insufficient direct data / indirect-channel watch |
| Thailand | Regional/religious cleavage track | Non-comparable | N/A for focal immigrant-origin measure | N/A | Not directly comparable | Strong regional/religious structure | Separate track | Separate track | Not classified | Regional/religious cleavage track |

## Current comparative patterns

### Pattern A — Alignment persistence / re-strengthening
Sweden.

### Pattern B — Historical alignment with fragmentation
Norway.

### Pattern C — Aggregate persistence with local subgroup fragmentation
Great Britain.

### Pattern D — Origin-dependent cross-bloc heterogeneity
Germany.

### Pattern E — Rapid partisan convergence through turnout composition
United States.

### Pattern F — Indirect counter-mobilization
Denmark and Italy, with strong context dependence and evidence that effects can weaken or differ by geography over time.

## Cross-country findings retained at v0.1

DDR-CF01 — Immigrant-origin electorates are not a single ideological bloc.  
Status: SUPPORTED.

DDR-CF02 — Left alignment exists in several receiving democracies but is not stable.  
Status: SUPPORTED WITH QUALIFICATION.

DDR-CF03 — Demographic effects can run in opposite directions through direct and indirect channels.  
Status: SUPPORTED AS A MECHANISM.

DDR-CF04 — Turnout can matter as much as preference.  
Status: SUPPORTED.

DDR-CF05 — "Immigrants decided the election" is too strong without counterfactual evidence.  
Status: METHODOLOGICAL RULE.

DDR-CF06 — Historical left alignment can persist without being stable.  
Status: SUPPORTED.

DDR-CF07 — Growth in eligible electorate does not equal proportional political impact.  
Status: SUPPORTED.

DDR-CF08 — Neighbouring systems can diverge despite similar migration-era pressures.  
Status: SUPPORTED DESCRIPTIVELY.

DDR-CF09 — Origin composition can matter more than broad migration-background status.  
Status: SUPPORTED.

DDR-CF10 — Aggregate alignment can coexist with constituency-level defection.  
Status: SUPPORTED.

DDR-CF11 — Demographic effects must be analysed at multiple geographic scales.  
Status: SUPPORTED.

DDR-CF12 — Realignment can occur through turnout composition rather than persuasion.  
Status: SUPPORTED.

DDR-CF13 — A large demographic electorate can lose partisan distinctiveness.  
Status: SUPPORTED.

DDR-CF14 — Outcome relevance is not identical to net partisan advantage.  
Status: SUPPORTED.

DDR-CF15 — Indirect electoral effects can oppose the direct immigrant-origin vote effect.  
Status: SUPPORTED.

DDR-CF16 — Counter-mobilization is context-dependent.  
Status: SUPPORTED.

DDR-CF17 — Demographic political effects can saturate, weaken or reverse.  
Status: SUPPORTED AS A MECHANISM.

## Falsification / stress conditions for the next pass

The working model must be weakened if additional comparable cases show that:

1. broad immigrant-origin status adds little explanatory value after origin, class, age, religion and geography are controlled;
2. direct alignment differences disappear over repeated elections;
3. turnout composition does not materially alter observed group-party relationships;
4. credible causal studies fail to replicate counter-mobilization outside the historical Denmark/Italy settings;
5. apparent demographic effects are better explained by geography, education, income, age or candidate effects;
6. outcome-relevance estimates prove highly unstable under reasonable assumptions;
7. countries with similar demographic change show no recurring mechanism once measurement differences are controlled.

## Next evidence pass

Priority:
- France: private-survey party-choice reconstruction remains open; turnout track retained.
- Poland: remain outside main module unless current host-country party-choice evidence emerges.
- Costa Rica: retain administrative electorate-structure track; seek survey vote-choice evidence.
- Chile: promote to partial core participation case; party-choice remains unresolved.
- Japan: retain indirect-channel watch only unless validated naturalized-citizen vote-choice data emerge.
- Ghana / Botswana / Thailand: keep separate cleavage tracks unless a defensible immigrant-origin comparison becomes available.

No additional country should be promoted into the main comparative finding set merely to fill the 15-country table.

NPCsignals  
Observe → Filter → Validate → Stress → Archive → Review
